...
module Vacancy
  class Role < ActiveRecord::Base
    attr_accessible :title, :subtitle, :description, :order, :contact_name, :contact_email,
                    :contact_phone, :active, :department_id, :image, :location, :created_at
    ...
    scope :in_department_and_active, -> { where(arel_table[:department_id].not_eq(nil).and(arel_table[:active].eq(true))) }
